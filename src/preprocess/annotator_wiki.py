...
class WikiAnnotator(BaseAnnotator):
    def __init__(self, preprocessor, use_cache, path_standard_phrase):
        ...
        self.path_tokenized_phrase = (
            self.dir_output / f"tokenized.{self.path_standard_phrase.name}"
        )
    ...
    def tokenize_phrases(self):
        ...
        phrases = [
            " " + p for p in phrases if len(p.split()) > 1
        ]  # for Roberta tokenizer
        tqdm_phrases = tqdm(
            phrases, ncols=100, desc=f"[WikiAnno] {self.path_standard_phrase}"
        )
        tokenized_phrases = [
            " ".join(consts.LM_TOKENIZER.tokenize(phrase, add_special_tokens=False))
            for phrase in tqdm_phrases
        ]
    ...
    def _mark_corpus(self):
        ...
        replaced_docs = [
            utils.Json.loads(line) for line in replaced_corpus.splitlines()
        ]
        assert len(raw_docs) == len(replaced_docs) == len(raw_id_docs)
        for raw_doc, raw_id_doc, replaced_doc in tqdm(
            zip(raw_docs, raw_id_docs, replaced_docs),
            ncols=100,
            total=len(raw_docs),
            desc="Clean phrases",
        ):
            ...
            for raw_sent, raw_id_sent, replaced_sent in zip(
                raw_sents, raw_id_sents, replaced_sents
            ):
                ...
                cleaned_phrases = [
                    p
                    for p in phrases
                    if p[0][0] in widxs_set
                    and (p[0][1] + 1 in widxs_set or p[0][1] == len(raw_tokens) - 1)
                ]
                raw_id_sent["phrases"] = cleaned_phrases
                num_cleaned_phrases = len(cleaned_phrases)
                num_partially_matched_phrases += (
                    num_original_phrases - num_cleaned_phrases
                )
