...
class AttmapModel(BaseModel):
    def __init__(
        ...
        self.cls_linear = nn.Linear(
            out_channels * self.width_after_pool2 * self.width_after_pool2, 1
        )
    ...
    def pad_attention_maps(attention_maps: list, max_num_subwords: int):
        ...
        padded_tensor = numpy.zeros(
            (num_instances, in_channels, max_num_subwords, max_num_subwords),
            dtype=numpy.float16,
        )
    ...
    def predict(
        ...
        path_predict_docs = self.feature_extractor.generate_predict_docs(
            path_tokenized_id_corpus
        )
        ...
        predict_docs = (
            predict_docs if max_num_docs is None else predict_docs[:max_num_docs]
        )
            ...
            for predict_doc in tqdm(predict_docs, ncols=100, desc="predict"):
                ...
                for predict_instance in predict_doc["sents"]:
                    ...
                    spans = [
                        span
                        for span in spans
                        if span[1] - span[0] + 1 <= self.max_num_subwords
                    ]
                    ...
                    for batch_spans in spans_batches:
                        span_attmaps = [
                            attmap[:, :, l_idx : r_idx + 1, l_idx : r_idx + 1]
                            for l_idx, r_idx, _ in batch_spans
                        ]
                        span_attmaps = self.pad_attention_maps(
                            span_attmaps, max_num_subwords=self.max_num_subwords
                        )
                ...
                predicted_docs.append(
                    {"_id_": predict_doc["_id_"], "sents": predicted_instances}
                )
